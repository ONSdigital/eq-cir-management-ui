...
from eq_cir_management_ui.utils.jinja_functions import create_error_panel_data, get_field_error
...
def jinja_config(app: Flask):
    ...
    file_system_loader = FileSystemLoader(
        [Path("./node_modules/@ons/design-system"), Path("./templates")]
    )
...
def design_system_config(app: Flask):
    ...
    with open(Path("./package.json")) as file:
        ...
        design_system_version = "".join(
            filter(lambda s: (s.isnumeric() or s == "."), design_system_version)
        )
...
def register_jinja_functions(app: Flask):
    """Register any functions that will be made available for use within the
    ...
    def register():
        return dict(
            ...
            create_error_panel_data=create_error_panel_data,
        )


def configure_secure_headers(app):
